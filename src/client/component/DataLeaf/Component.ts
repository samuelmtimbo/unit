...
export default class DataTreeLeaf extends Element<HTMLDivElement, Props> {
  ...
  private _on_keydown = (
    ...
    const selectionStart = this._input.$element.selectionStart
    this.dispatchEvent(
      'leafkeydown',
      {
        value,
  ...
  private _on_focus = (event: IOFocusEvent): void => {
    const { path, value } = this.$props
    this.dispatchEvent('leaffocus', { path, value })
  }

  private _on_blur = (event: IOFocusEvent): void => {
    const { path, value } = this.$props
    this.dispatchEvent('leafblur', { path, value })
  }
