...
export default class DataTree extends Element<HTMLDivElement, Props> {
  ...
  private __primitive = () => this._primitive()
  private __key_value = () => this._key_value()
  private __array_expression = () => this._array_expression()
  private __object_expression = () => this._object_expression()
  private __expression = () => this._expression()
  private __object_literal = () => this._object_literal()
  private __array_literal = () => this._array_literal()
  private __and = () => this._and()
  private __or = () => this._or()

  private NODE_TYPE_TO_ELEMENT = {
  ...
  private _reset = () => {
    ...
    const element = (
      this.NODE_TYPE_TO_ELEMENT[data.type] ||
      (() => ({ style: {}, children: [] }))
    )()

    // let element: { style: Dict<string>; children: Element[] }
    ...
    //     element = this._primitive()
    //     break
    //   case TreeNodeType.KeyValue:
    //     element = this._key_value()
    //     break
    //   case TreeNodeType.ArrayExpression:
    //     element = this._array_expression()
    //     break
    //   case TreeNodeType.ObjectExpression:
    //     element = this._object_expression()
    //     break
    //   case TreeNodeType.Expression:
    //     element = this._expression()
    //     break
    //   case TreeNodeType.ObjectLiteral:
    //     element = this._object_literal()
    //     break
    //   case TreeNodeType.ArrayLiteral:
    //     element = this._array_literal()
    //     break
  ...
  private _object_literal = (): {
    ...
    const { data, appendChildren = [], parent } = this.$props
    ...
    for (let i = 0; i < data.children.length; i++) {
      ...
      if (child.type === TreeNodeType.KeyValue) {
        ...
        mergeStyle(key_tree, { display: 'flex' })
  ...
  private _expression = (): {
    ...
    const { data, path = [], appendChildren = [] } = this.$props
    const empty = data.children.length === 0
    const overflow = childrenOverflow(data)
    const style = STYLE_PARENT(overflow)
  ...
  private _key_value = (): { style: Dict<string>; children: Element[] } => {
    const { data, appendChildren = [] } = this.$props
  ...
  private _primitive = (): { style: Dict<string>; children: Element[] } => {
    const { data, path = [], parent, appendChildren = [] } = this.$props
  ...
  private _or = (): { style: Dict<string>; children: Element[] } => {
    const { data } = this.$props
    const overflow = childrenOverflow(data)
    const style = STYLE_PARENT(overflow)
  ...
  private _and = (): { style: Dict<string>; children: Element[] } => {
    const { data } = this.$props
    const style = { display: 'flex' }
    const children: Element[] = []
    for (let i = 0; i < data.children.length; i++) {
