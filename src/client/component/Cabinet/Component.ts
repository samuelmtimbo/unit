...
import { SimNode, Simulation } from '../../simulation'
...
export default class Cabinet extends Element<HTMLDivElement, Props> {
  ...
  onPropChanged(prop: string, current: any): void {
    if (prop === 'className') {
      ...
      const drawers = this._drawer_component
      for (const drawer_id in drawers) {
        const drawer = drawers[drawer_id]
        drawer.setProp('hidden', current)
      }
    }
  }
  ...
  private _on_resize = ($width: number, $height: number) => {
    ...
    for (const drawer_id in this._drawer_component) {
      ...
      drawer_component.setProp('y', y + dy)
