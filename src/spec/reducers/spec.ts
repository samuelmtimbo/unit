...
export const setName = ({ name }: { name: string }, state: State): State =>
  assoc(state, 'name', name) as State
...
export const addUnits = (
  { units }: { units: UnitsSpec },
  state: State
): State => assoc(state, 'units', merge(state.units, units)) as State
...
export const addMerges = (
  ...
  return assoc(state, 'merges', deepMerge(merges, state.merges!)) as State
}
...
export const removeMerge = ({ id }: { id: string }, state: State): State => {
  ...
  let nextInputs = mapObj<GraphExposedPinSpec, GraphExposedPinSpec>(
    state.inputs,
  ...
  let nextOutputs = mapObj<GraphExposedPinSpec, GraphExposedPinSpec>(
    state.outputs,
