...
export class Graph<I = any, O = any> extends Primitive implements G, C, U {
  ...
  public _coverPinSet = (type: 'input' | 'output', id: string): void => {
    this._simCoverPinSet(type, id)
    this._memCoverPinSet(type, id)
  }

  private _memCoverPinSet = (type: 'input' | 'output', pinId: string): void => {
  ...
  private _simCoverPinSet = (type: 'input' | 'output', pinId: string): void => {
    const exposedNodeId = getExposedPinId(pinId, type)
  ...
  public moveLinkPinInto(
    ...
    this._moveLinkPinInto(graphId, unitId, type, pinId)
  ...
  public moveMergeInto(
    ...
    const merge_spec = this.getMergeSpec(mergeId)
    const mergeUnitCount = _keyCount(merge_spec)
    const mergePinCount = this.getMergePinCount(mergeId)

    if (mergeUnitCount === 1) {
      const mergeSingleUnitId = getObjSingleKey(merge_spec)

      if (mergeSingleUnitId === graphId) {
        const mergeSingleUnit = merge_spec[mergeSingleUnitId]
