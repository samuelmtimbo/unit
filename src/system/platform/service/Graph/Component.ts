...
import Minigraph from '../../component/core/Minigraph/Component'
import Modes from '../../component/core/Modes/Component'
import Div from '../../component/Div/Component'
import { enable_mode_keyboard } from '../../component/graph/Graph/Component'
...
export default class GraphService extends Component<HTMLDivElement, Props> {
  private _root: Div
      ...
      list.$element.addEventListener(
        ...
        ({ detail: { pointerId, pointerType } }: CustomEvent<PointerEvent>) => {
          if (this._new_spec_item) {
            ...
            this._remove_new_spec_item()
            const spec_id = uuid()
            const bundle = this.$system.flag.dragAndDrop[pointerId]
  ...
  private _render_spec_item = (
    ...
    spec_item.addEventListener(
      makePointerEnterListener(({ pointerId }) => {
        ...
        if (!this.$system.flag.dragAndDrop[pointerId]) {
