...
import { connect as web } from '../../../../client/host/service/web'
import {
  addSignInListener,
  addSignOutListener,
  isSignedIn,
...
import { SharedObject, SharedObjectClient } from '../../../../SharedObject'
...
import GraphService from '../Graph/Component'
import IOTService from '../IOT/Component'
...
export default class Cloud extends Component<HTMLDivElement, Props> {
  private _root: Div
  ...
  onMount() {
    ...
    for (const service of ['graph', 'web', 'vm']) {
      const store = STORE[service]()

      this._store_client[service] = {
        user: store['user'].connect(),
