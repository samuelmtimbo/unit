...
export default class Bot extends Element<SVGSVGElement, Props> {
  private _random_id: string
  ...
  constructor($props: Props) {
    ...
    applyStyle(svg, { ...DEFAULT_STYLE, ...style })
    this._svg = svg
  ...
  private _tick_body($context: Context) {
    ...
    for (let pointerId in this._pointer_down) {
      const p = this._pointer_position[pointerId]
      ...
      if (!laser) {
        ...
        // laser_focus.style.transform = 'translate(-50%, -50%)'
  ...
  private _getXY = (event: IOPointerEvent): Position => {
    ...
    const x = (screenX - $x) / $sx
    const y = (screenY - $y) / $sy
    return { x, y }
  }
  ...
  private _tick_color = (): void => {
    // console.log('Bot', '_refresh_color')
    ...
    for (let i = 0; i < this._eye_ellipse.length; i++) {
      ...
      if (
        ...
        this._pointing_self_count === this._pointer_down_count
      ) {
  ...
  onPropChanged(prop: string, current: any) {
    ...
    if (prop === 'style') {
      applyStyle(this._svg, { ...DEFAULT_STYLE, ...current })
      this._tick_color()
    }
    if (prop === 'disabled') {
      this._refresh_enabled()
