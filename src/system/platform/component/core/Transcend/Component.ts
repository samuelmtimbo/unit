...
export const DEFAULT_STYLE = {
  ...
  // borderTop: 'none',
  marginTop: '-4px',
  // transition: 'top ${ANIMATION_T}s linear',
...
export default class Transcend extends Element<HTMLDivElement, Props> {
  ...
  public up(animate: boolean = true) {
    ...
    this._down = false
    mergeStyle(this._icon, {
      transform: `rotate3d(1, 0, 0, 0deg)`,
      transition: animate ? `transform ${ANIMATION_T}s linear` : '',
    })
  }
  ...
  private _animate_y = (target_y: number): void => {
    ...
    const k = ((dy0 / 12) * ANIMATION_T) / 0.2
