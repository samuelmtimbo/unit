...
import Minimap from '../../../../../client/component/Minimap/Component'
import { Element } from '../../../../../client/element'
import { getLinkId, isComponent, segmentLinkId } from '../../../../../client/id'
...
import { NOT_SUBGRAPH_MAX_D, SUBGRAPH_MAX_D } from '../../graph/Graph/Component'
...
export default class Mingraph extends Element<HTMLDivElement, Props> {
  ...
  private _reset = (): void => {
    ...
    const { spec = emptyGraphSpec } = this.$props

    const { units = {}, merges } = spec

    const nodes: Dict<SimNode<any>> = mapObj(units, (unit_spec, id) => {
      this._node_count++

      const { path, metadata = {} } = unit_spec

      const { position = { x: 0, y: 0 }, component = {} } = metadata

      const { x, y } = position

      let r = getSpecRadius(path, true)
      let width = 2 * r
      let height = 2 * r

      const is_component = isComponent(path)
