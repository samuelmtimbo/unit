...
import { getPosition } from '../../../../../client/getPosition'
...
import { isComponent, isComponentSpec } from '../../../../../client/spec'
...
function extractStyle($element: IOElement): Style {
  const style = {}
...
export default class GraphComponent extends Element<HTMLElement, Props> {
  ...
  private _on_compose = () => {
    ...
    this._graph.enter_subgraph(parent_unit_id)
    this._graph.leave()
...
export class _GraphComponent extends Element<HTMLDivElement, _Props> {
  ...
  private _layout_transfer_animation_current_parent_id: string | null = null

  private _layout_transfer_parent_animating: Dict<boolean> = {}

  private _layout_transfer_parent_leaf: Dict<[string[], Component][]> = {}
  private _layout_transfer_parent_leaf_sub_component_id: Dict<string[]> = {}
  private _layout_transfer_parent_leaf_path: Dict<string[][]> = {}
  private _layout_transfer_parent_leaf_comp: Dict<Component[]> = {}
  private _layout_transfer_parent_leaf_count: Dict<number> = {}
  private _layout_transfer_parent_leaf_end_count: Dict<number> = {}
  private _layout_transfer_parent_leaf_end_set: Dict<Set<string>> = {}
  private _layout_transfer_parent_leaf_slot_style: Dict<Style> = {}
  private _layout_transfer_parent_leaf_style: Dict<Style[]> = {}

  private _layout_transfer_parent_remaining_child: Dict<Set<string>> = {}
  ...
  private _exposed_pin_name: Dict<TextInput> = {}
  ...
  constructor($props: _Props) {
    ...
    main.$element.addEventListener('_dragdrop', (event: CustomEvent) => {
      ...
      const spec = this.$system.flag.dragAndDrop[pointerId]
  ...
  private _get_spec_init_unit_pin_position = (
    ...
    this._for_each_spec_id_pin(path, (type: IO, pin_id: string) => {
  ...
  private _get_sub_component_root_children = (
    sub_component_id: string
  ): Component[] => {
    console.log('Graph', '_get_sub_component_root_children', sub_component_id)

    const sub_component = this._get_sub_component(sub_component_id)
    const sub_component_root_children = sub_component.$root
    return sub_component_root_children
  }

  private _get_sub_component_sub_components = (
    sub_component_id: string
  ): Dict<Component> => {
    console.log('Graph', '_get_sub_component_sub_components', sub_component_id)

    const sub_component = this._get_sub_component(sub_component_id)
    const sub_component_sub_components = sub_component.$subComponent
    return sub_component_sub_components
  }
  ...
  private _animate_sub_sub_components = (sub_component_id: string): void => {
    // const sub_sub_components =
    //   this._get_sub_component_sub_components(sub_component_id)

    const sub_component_root_children =
      this._get_sub_component_root_children(sub_component_id)

    for (const sub_component_root of sub_component_root_children) {
      this._animate_sub_component_root(sub_component_root)
    }

    // const sub_component_root_base = this._get_sub_component_root_base(sub_component_id)

    // for (const sub_component_root_leaf of sub_component_root_base) {
    //   const [sub_component_root_leaf_path, sub_component_root_leaf_component] = sub_component_root_leaf
    //   console.log(sub_component_root_leaf_path, sub_component_root_leaf_component)
    // }
  }

  private _animate_sub_component_root = (sub_component_root: Component) => {
    const sub_component_root_base = sub_component_root.getRootBase()
  }
  ...
  private _sim_add_unit_core = (
    ...
    if (is_component) {
      ...
      r = getSpecRadiusById(specs, path, true)
      width = 2 * r
      height = 2 * r
      shape = 'circle'
    }
    ...
    const { width: name_width, height: name_height } =
      this._get_core_name_size(unit_id)
  ...
  private _set_core_selection_to_core = (unit_id: string): void => {
    const {
      width: core_width,
      height: core_height,
      shape: core_shape,
    ...
    selection.setProp('shape', core_shape)

    this._resize_core_selection(unit_id, core_width, core_height)
  }

  private _set_core_selection_to_name = (unit_id: string): void => {
    const { height: core_height } = this._node[unit_id]

    const selection = this._node_selection[unit_id]

    const { width, height } = this._get_core_name_size(unit_id)

    selection.setProp('y', core_height / 2 + 3 + height / 2)
    selection.setProp('shape', 'rect')

    this._resize_core_selection(unit_id, width - 4 - 2, height - 4 - 2)
  }
  ...
  private _set_unit_temp_name = (unit_id: string, value: string): void => {
    ...
    if (value.length > UNIT_NAME_MAX_SIZE) {
      value = value.substr(0, UNIT_NAME_MAX_SIZE)

      this._node_name[unit_id] = value
    ...
    if (valid) {
      ...
      core_name.$element.style.caretColor = OPAQUE_RED
    }
  ...
  private _get_core_name_size = (unit_id: string): Size => {
    const value = this._node_name[unit_id]

    // console.log('Graph', '_get_core_name_size', unit_id, value)

    const { width, height } = getDivTextSize(
      value,
  ...
  private _on_core_name_blur = (unit_id: string): void => {
    ...
    if (selection) {
      ...
      if (core_name.$element.contains(anchorNode)) {
    ...
    this._set_core_selection_to_core(unit_id)
  }
  ...
  private _set_node_temp_name = (node_id: string, value: string): void => {
    ...
    if (this._is_unit_node_id(node_id)) {
      ...
      this._set_external_pin_temp_name(node_id, value)
    }
  }

  private _set_external_pin_temp_name = (
    ...
    this._set_exposed_pin_name(ext_pin_node_id, value)

    // AD HOC
    // node will be deleted (blur) and added again with a new id
    const { type, id, subPinId } = segmentExposedNodeId(ext_pin_node_id)
  ...
  private _sim_add_exposed_pin = (
    ...
    this._exposed_pin_name[ext_pin_node_id] = pin_name
    ...
    if (input) {
      ...
      end_marker = new SVGPath({ d: end_d, style: marker_style })
    } else {
      const start_d = ARROW_SHAPE
      start_marker = new SVGPath({ d: start_d, style: marker_style })
    }
  ...
  private _create_layout_layer = ({
    ...
    return { layer, background, height, children, layers }
  }
  ...
  private _sim_add_link_pin_node = (
    ...
    if (ref_output) {
      ...
      if (pin) {
        ...
        const unit_r = getSpecRadiusById(specs, path)
        if (unit_component) {
          shape = 'rect'
        }
        // r = 0.8 * unit_r
  ...
  private _tick_node = (node_id: string): void => {
    ...
    if (this._node_target_count > 0) {
      for (const node_id in this._node_target) {
        ...
        if (this._is_internal_pin_node_id(node_id)) {
    ...
    if (this._drag_ext_node_count > 0) {
      for (const ext_node_id of this._drag_ext_node_id) {
        const { type, id, subPinId } = segmentExposedNodeId(ext_node_id)

        const a_id = getInternalNodeId(type, id, subPinId)
  ...
  private _is_node_moded = (node_id: string): boolean => {
    if (
      ...
      if (this._is_link_pin_node_id(node_id)) {
        ...
        if (datum_pin_node_id) {
          if (this._is_node_moded(datum_pin_node_id)) {
            return true
          }
        }
      }
      ...
      if (search_unit_id) {
        ...
        const replace_exposed = (type: IO, tag: 'ref' | 'data'): void => {
          ...
          for (const pin_id in exposed_pins) {
            ...
            this._state_plug_exposed_pin(type, id, subPinId, sub_pin_spec)
          }
        }
    ...
    if (unit_id) {
      if (this._mode === 'add' || this._mode === 'change') {
        ...
        if (this._is_unit_component(unit_id)) {
          ...
          if (parent_id) {
            this._mem_push_sub_component_child(parent_id, unit_id, 'default')
          }
        }
  ...
  private _animate_frame = (
    ...
    return this.__animate_frame(frame, init, f, N, callback)
  }

  private __animate_frame = (
    frame: Frame,
    {
      x0,
      y0,
    ...
    if (this._in_component_control) {
      ...
      if (
        ...
        for (const sub_component_id of sub_component_ids) {
          ...
          const N = Math.floor(animate ? (12 * ANIMATION_T_S) / 0.2 : 0)
  ...
  private _animate_layout_core = (
    ...
    this._layout_core_abort_animation[sub_component_id] =
      this._animate_simulate(
        ...
        (o) => {
          // this._set_layout_core_opacity(sub_component_id, k0)
        },
        () => {
          delete this._layout_core_abort_animation[sub_component_id]
  ...
  private _animate_leaf_frame = (
    ...
    return this._animate_simulate(
      ...
      (w, h) => {
        leaf_frame.$element.style.width = `${w}px`
        leaf_frame.$element.style.height = `${h}px`
  ...
  private _animate_simulate = (
    ...
    set_scale: (k: number) => void,
    set_opacity: (opacity: number) => void,
    callback: () => void
  ): Callback => {
    // console.log('Graph', '_animate_simulate')

    let { x, y, width: w, height: h, k, opacity: op } = n0
    ...
    const animate = () => {
      const { x: x1, y: y1, width: w1, height: h1, k: k1, opacity: op1 } = n1()
  ...
  private _animate_layout_core_up = (
    ...
    this._animate_layout_core(
      sub_component_id,
      { x, y, width: w0, height: h0, k: k0, opacity: 1 },
      () => {
  ...
  private _get_leaf_comp_trait = (leaf_comp: Component): LayoutNode => {
    ...
    return { x, y, width, height, k: 1, opacity: 1 }
  }
  ...
  private _plug_leaf_frame = (
    ...
    const { x, y, width, height, k } = leaf_node
  ...
  private _get_sub_component_parent_leaf_layer = (parent_id: string): Div => {
    ...
    const { background: parent_background } = parent_layer
    const { background: parent_parent_background } = parent_parent_layer

    const leaf_layer = is_parent_uncollapsed
      ? parent_background
      : parent_parent_background
  ...
  private _measure_sub_component_base = (sub_component_id: string): void => {
    ...
    for (const leaf of base) {
      ...
      const leaf_id = this._get_leaf_id(sub_component_id, leaf_path)

      const leaf_node = this._get_leaf_comp_trait(leaf_comp)
  ...
  private _remove_sub_component_base = (sub_component_id: string): void => {
    ...
    for (const leaf of base) {
      ...
      const leaf_parent = sub_component.getPathSubComponent(leaf_parent_path)

      if (leaf_parent === leaf_comp) {
        if (frame.$children.includes(leaf_comp)) {
          frame.removeChild(leaf_comp)
        } else {
          //
        }
      } else {
  ...
  private _reflect_sub_component_base_trait = (
    ...
    const frame_style = extractStyle(sub_component_frame.$element)

    const all_leaf_style = []

    const sub_sub_component_id_slot: Dict<string> = {}
    ...
    const root_leaf_id = []
    const slot_leaf_id: Dict<string[]> = {}

    const leaf_slot: Dict<string | null> = {}

    const all_slot_style = {}

    const all_slot_trait = {}

    const all_leaf_trait = {}

    for (const leaf of base) {
      const [leaf_path, leaf_comp] = leaf

      const sub_sub_component_id = leaf_path[0]
      ...
      const is_sub_sub_component_root =
        sub_component.$root.includes(sub_sub_component) ||
        sub_component === leaf_comp

      const leaf_style = extractStyle(leaf_comp.$element)

      all_leaf_style.push(leaf_style)

      if (is_sub_sub_component_root) {
        root_sub_sub_component_id.push(sub_sub_component_id)
        ...
        const sub_sub_parent_root_index =
          sub_component.$parentRoot.indexOf(sub_sub_component)
        const sub_sub_component_slot_name =
          sub_component.$parentRootSlot[sub_sub_parent_root_index]

        sub_sub_component_id_slot[sub_sub_component_id] =
          sub_sub_component_slot_name

        all_slot_style[sub_sub_component_slot_name] =
          all_slot_style[sub_sub_component_slot_name] || []
        all_slot_style[sub_sub_component_slot_name].push(leaf_style)

        slot_leaf_id[sub_sub_component_slot_name] =
          slot_leaf_id[sub_sub_component_slot_name] || []
        slot_leaf_id[sub_sub_component_slot_name].push(leaf_id)

        leaf_slot[leaf_id] = sub_sub_component_slot_name
      }
    }
    ...
    for (const slot_name in sub_component.$slot) {
      const slot = sub_component.$slot[slot_name]

      let slot_offset_element = slot.$element as IOElement
      while (
        slot_offset_element &&
        slot_offset_element.classList.contains('__parent')
      ) {
        slot_offset_element = slot_offset_element.parentElement
      }

      slot_offset_element = slot_offset_element || frame.$element

      const slot_style = extractStyle(slot_offset_element)

      const slot_size = getSize(slot_offset_element)

      const slot_all_leaf_style = all_slot_style[slot_name] || []

      const slot_trait = reflectChildrenTrait(
        slot_size.width,
        slot_size.height,
        slot_style,
        slot_all_leaf_style
      ...
      all_slot_trait[slot_name] = slot_trait
    }
    ...
    for (const slot_name in slot_leaf_id) {
      const _slot_leaf_id = slot_leaf_id[slot_name]

      let slot_leaf_i = 0
      for (const leaf_id of _slot_leaf_id) {
        all_leaf_trait[leaf_id] = all_slot_trait[slot_name][slot_leaf_i]
        slot_leaf_i++
      }
    }
      ...
      if (!visible && children.length > 0) {
        ...
        for (const child_id of children) {
          if (!this._animating_sub_component_base_id.has(child_id)) {
            ...
            this._remove_sub_component_base(child_id)
          }
        }
        ...
        this._abort_tree_layout_sub_component_base_animation[sub_component_id] =
          this._animate_sub_component_base(
            ...
            () => {
              ...
              this._layout_sub_component_commit_base(sub_component_id)
            }
          )
      }
    }
        ...
        this._animate_layout_sub_component_remove_children(
          ...
          () => {
            for (const child_id of children) {
              ...
              this._layout_sub_component_commit_base(child_id)
            }
          }
        )
      }
  ...
  private _animate_layout_core_down = (
    ...
    this._animate_layout_core(
      ...
      () => {
        ...
        return { x, y, width, height, k, opacity: 1 }
      },
      callback
    )
  }
  ...
  private _drop_exposed_pin = (exposed_pin_node_id: string): void => {
    ...
    if (!this._exposed_ext_plugged[exposed_ext_node_id]) {
      ...
      if (nearest_compatible_node_id) {
        this._set_node_target(exposed_int_node_id, nearest_compatible_node_id)
      } else {
        this._refresh_node_layer(exposed_ext_node_id)
        this._refresh_node_layer(exposed_int_node_id)
      }
    } else {
      this._refresh_node_layer(exposed_ext_node_id)
    }
  ...
  private _drop_commit_exposed_pin(
    ...
    if (this._is_pin_node_id(target_node_id)) {
      this.__plug_exposed_pin_to(type, id, subPinId, target_node_id)
  ...
  private _enable_plug_name = (ext_node_id: string): void => {
    // console.log('Graph', '_disable_plug_name', unit_id)

    const plug_name = this._exposed_pin_name[ext_node_id]

    plug_name.$element.style.pointerEvents = 'inherit'
  }
  ...
  private _disable_plug_name = (ext_node_id: string): void => {
    ...
    plug_name.$element.style.pointerEvents = 'none'
  }
  ...
  private _animate_sub_component_graph_enter = (
    x: number,
    y: number,
    width: number,
    height: number,
    ...
    console.log(
      'Graph',
      '_animate_sub_component_graph_enter',
      x,
      y,
      width,
      height,
  ...
  private _animate_sub_component_graph_leave = (
    ...
    const leaf_layer = this._sub_between
    ...
    const frame = this._get_sub_component_frame(sub_component_id)
    const sub_component_context = this._get_sub_component_frame_context(sub_component_id)

    const all_leaf_trait = this._reflect_sub_component_base_trait(
      sub_component_id,
      base,
      frame,
      sub_component_context.$width,
      sub_component_context.$height
    )
  ...
  public remove_exposed_sub_pin = (exposed_pin_node_id: string): void => {
    ...
    this._dispatch_action(coverPin(id, type, subPinId))
  }
  ...
  private _sim_remove_exposed_ext_node = (ext_pin_node_id: string) => {
    ...
    delete this._exposed_pin_name[ext_pin_node_id]
  ...
  private _on_unit_blue_click = (unit_id: string): void => {
    ...
    this._for_each_unit_pin(unit_id, (pin_node_id, kind, pin_id) => {
      ...
      if (int_pin_node_id) {
        ...
        this._search_unit_exposed_pin_ids[kind][tag][pin_id] = [id, subPinId]
  ...
  public set_exposed_pin_functional = (
    ...
    const { type, id } = segmentExposedNodeId(exposed_node_id)
    this._set_exposed_pin_functional(exposed_node_id, functional)
    this._dispatch_action(setPinSetFunctional(type, id, functional))
  }
  ...
  public enter_subgraph = (unit_id: string): void => {
  ...
  private _enter_subgraph = (unit_id: string): void => {
    ...
    let opt: Dict<{ base: LayoutBase; base_node: LayoutNode[] }> = {}
    ...
    if (!isBaseSpecId(id)) {
      ...
      if (is_component) {
        ...
        // opt = this._get_sub_component_base_trait(unit_id)
        for (const sub_sub_component_id in sub_component.$subComponent) {
          ...
          for (const sub_sub_component_leaf of base) {
            ...
            const { x, y } = leaf_comp.getElementPosition()
            const { width, height } = leaf_comp.getElementSize()
  ...
  public enter = (
    ...
    if (opt) {
      ...
      for (const sub_component_id in this._component.$subComponent) {
        const { base, base_node } = opt[sub_component_id]
  ...
  public leave = (): void => {
    ...
    if (this._subgraph_unit_id) {
      ...
      if (parent) {
        ...
        const sub_base = this._component.getRootBase()
        const sub_base_node = []

        for (const leaf of sub_base) {
          const [leaf_path, leaf_comp] = leaf

          const position = leaf_comp.getElementPosition()
          const size = leaf_comp.getElementSize()
  ...
  private _get_sub_component_base_trait(sub_component_id: string): {
    ...
    for (const sub_component_base_leaf of base) {
      const [_, sub_component_base_leaf_comp] = sub_component_base_leaf

      const leaf_element = sub_component_base_leaf_comp.$element

      const leaf_relative_position = getPosition(leaf_element, context.$element)

      const { x, y } = addVector(context_position, leaf_relative_position)

      const { width, height } = getSize(leaf_element)
  ...
  private _fake_bool = true

  private _layout_sub_component_commit_base = (
    sub_component_id: string
  ): void => {
    ...
    if (animate) {
      ...
      for (const child_id of children) {
        this._animate_layout_core_anchor(
          ...
          this._layout_node[child_id],
          () => {
            this._layout_move_sub_component_child(
              current_layer_id,
              parent_id,
              child_id
            )
  ...
  private _pre_append_sub_component_child = (
    ...
    this._mem_move_sub_component_child(parent_id, child_id, slot_name)
    ...
    if (
      ...
      (!this._layout_transfer_parent_remaining_child[parent_id] ||
        !this._layout_transfer_parent_remaining_child[parent_id].has(child_id))
    ) {
      // this._leave_sub_component_frame(child_id)
    }
  }

  private _get_leaf_id = (
    sub_component_id: string,
    leaf_path: string[]
  ): string => {
  ...
  private _setup_layout_sub_component_append_children_animation = (
    ...
    this._reset_layout_transfer_parent(parent_id)

    this._layout_transfer_animation_current_parent_id = parent_id
    ...
    let slot_offset_element = slot.$element as IOElement
    while (slot_offset_element.classList.contains('__parent')) {
      slot_offset_element = slot_offset_element.parentElement
    }

    const _slot_style = extractStyle(slot_offset_element)
    const _leaf_style = []
    ...
    this._reset_layout_transfer_parent_children(parent_id, children)
    ...
    for (const child_id of children) {
      ...
      for (const leaf of base) {
        this._layout_transfer_parent_leaf[parent_id].push(leaf)
        this._layout_transfer_parent_leaf_sub_component_id[parent_id].push(
          child_id
        )

        const [leaf_path, leaf_comp] = leaf

        this._layout_transfer_parent_leaf_path[parent_id].push(leaf_path)
        this._layout_transfer_parent_leaf_comp[parent_id].push(leaf_comp)
      }
    ...
    this._layout_transfer_parent_leaf_slot_style[parent_id] = _slot_style
    this._layout_transfer_parent_leaf_style[parent_id] = _leaf_style

    let leaf_i = 0

    this._layout_transfer_parent_remaining_child[parent_id] = new Set(children)
    ...
    for (const child_id of children) {
      ...
      for (const leaf of base) {
        ...
        const leaf_id = this._get_leaf_id(child_id, leaf_path)
    ...
    this._layout_transfer_parent_leaf_count[parent_id] = leaf_i
  }
  ...
  private _setup_layout_sub_component_remove_parent_children_animation = (
    ...
    for (const child_id of children) {
      ...
      this._layout_transfer_parent_leaf_slot_style[parent_id] = slot_style
      ...
      this._layout_transfer_parent_leaf_style[child_id] = child_base_style
    }

    this._layout_transfer_parent_leaf_style[parent_id] = parent_child_leaf_style

    let leaf_i = 0

    for (const leaf of this._layout_transfer_parent_leaf[parent_id]) {
      const sub_component_id =
        this._layout_transfer_parent_leaf_sub_component_id[parent_id][leaf_i]
  ...
  private _cancel_layout_current_children_transfer_animation = (): void => {
    this._cancel_layout_parent_children_animation(
      this._layout_transfer_animation_current_parent_id
    )
  }
  ...
  private _cancel_layout_parent_children_animation = (
    ...
    if (this._layout_animation_frame[parent_id] !== undefined) {
      this._layout_transfer_animation_current_parent_id = null
  ...
  private _animate_layout_append_children = (
    ...
    const frame = () => {
      ...
      if (
        ...
        this._start_layout_children_animation(parent_id, frame)
      } else {
        this._cancel_layout_parent_children_animation(parent_id)
        ...
        for (
          let i = 0;
          i < this._layout_transfer_parent_leaf[parent_id].length;
          i++
        ) {
          const child_id =
            this._layout_transfer_parent_leaf_sub_component_id[parent_id][i]
          const leaf_path = this._layout_transfer_parent_leaf_path[parent_id][i]
          const leaf_comp = this._layout_transfer_parent_leaf_comp[parent_id][i]
  ...
  private _play_layout_sub_component_remove_children_animation = (
    ...
    const frame = () => {
      this._tick_animate_layout_parent_remove_children(parent_id, children)
  ...
  private _tick_animate_layout_append_children = (
    ...
    const anchor_id = this._get_layout_node_anchor_id(parent_id)

    const parent_component = this._get_sub_component(parent_id)
    const anchor_component = this._get_sub_component(anchor_id)

    const slot = parent_component.$slot[slot_name]

    let slot_offset_element = slot.$element as HTMLElement
    while (slot_offset_element.classList.contains('__parent')) {
      slot_offset_element = slot_offset_element.parentElement
    }
    ...
    const target_children_trait = reflectChildrenTrait(
      slot_size.width,
      slot_size.height,
    ...
    for (const child_id of children) {
      const parent_context = this._get_sub_component_frame_context(parent_id)
      const anchor_context = this._get_sub_component_frame_context(anchor_id)

      let target_context: Context = parent_context

      if (anchor_component.$element.contains(parent_component.$element)) {
        target_context = anchor_context
      }

      const target_context_position = {
        x: target_context.$x,
        y: target_context.$y,
      }
      ...
      const slot_context = slot.$context

      const slot_context_position = {
        x: slot_context.$x,
        y: slot_context.$y,
      }

      const sub_component_root_base =
        this._get_sub_component_root_base(child_id)

      for (const leaf of sub_component_root_base) {
        const [leaf_path, leaf_comp] = leaf

        const leaf_id = `${child_id}-${leaf_path.join('/')}`

        if (this._layout_transfer_parent_leaf_end_set[parent_id].has(leaf_id)) {
          this._layout_transfer_parent_leaf_end_count[parent_id]--
          this._layout_transfer_parent_leaf_end_set[parent_id].delete(leaf_id)
        }

        const leaf_i = leaf_count

        const leaf_node = this._leaf_frame_node[leaf_id]

        const { x, y, width, height, opacity, k } =
          this._leaf_frame_node[leaf_id]

        const leaf_frame = this._leaf_frame[leaf_id]

        const slot_relative_position = getPosition(
          slot.$element,
          slot_context.$element
        )

        const slot_position = addVector(
          slot_context_position,
          slot_relative_position
        )

        const target_child_trait = target_children_trait[leaf_i]
        ...
        const relative_position = { x: target_x, y: target_y }
        const target_position = addVector(slot_position, relative_position)

        const target_size = { width: target_width, height: target_height }
        ...
        const dw = target_size.width - width
        const dh = target_size.height - height
        ...
        if (Math.abs(dx) > 1 || Math.abs(dy) > 1) {
          ...
          leaf_frame.$element.style.left = `${_x}px`
          leaf_frame.$element.style.top = `${_y}px`
        ...
        if (Math.abs(dw) > 1 || Math.abs(dh) > 1) {
          ...
          const _w = width + dw * c
          const _h = height + dh * c

          leaf_frame.$element.style.width = `${_w}px`
          leaf_frame.$element.style.height = `${_h}px`
        ...
        if (ended) {
          this._layout_transfer_parent_leaf_end_count[parent_id]++
          this._layout_transfer_parent_leaf_end_set[parent_id].add(leaf_id)
        }
  ...
  private _tick_animate_layout_parent_remove_children = (
    ...
    const _children = this._layout_transfer_parent_remaining_child[parent_id]

    for (const child_id of _children) {
      const context = this._get_sub_component_frame_context(child_id)

      const slot_style = extractStyle(context.$element)
      const slot_position = { x: context.$x, y: context.$y }
      const slot_size = { width: context.$width, height: context.$height }

      const leaf_style = this._layout_transfer_parent_leaf_style[child_id]
      ...
      for (const leaf of base) {
        ...
        const { x, y, width, height, k, opacity } = leaf_node

        const leaf_frame = this._leaf_frame[leaf_id]

        const target_child_trait = target_children_trait[leaf_count]
        ...
        const target_relative_position = { x: target_x, y: target_y }
        const target_size = { width: target_width, height: target_height }

        const target_position = addVector(
          slot_position,
  ...
  private _mem_push_child = (child_id: string) => {
    const child_component = this._get_sub_component(child_id)
  ...
  private _reset_layout_transfer_parent = (parent_id: string): void => {
    ...
    this._layout_transfer_parent_leaf_end_count[parent_id] = 0
    this._layout_transfer_parent_leaf_end_set[parent_id] = new Set()
  }
  ...
  private _move_out_sub_component_children = (
    parent_id: string,
    next_parent_id: string | null
  ): void => {
    const children = this._get_sub_component_spec_children(parent_id)
    ...
    for (const child_id of children) {
      ...
      this._layout_move_sub_component_child(parent_id, next_parent_id, child_id)
    }
  }
  ...
  private _mem_move_out_sub_component_child = (
    ...
    // console.log(
    //   'Graph',
    //   '_mem_move_out_sub_component_child',
    //   parent_id,
    //   child_id,
    //   next_parent_id
    // )
  ...
  private _sim_add_pin_to_merge(pin_node_id: string, merge_node_id: string) {
    ...
    if (int_node_id) {
      const { id, type, subPinId } = segmentExposedNodeId(int_node_id)
      this._sim_unplug_exposed_pin(type, id, subPinId)
      this._sim_plug_exposed_pin(type, id, subPinId, { mergeId: merge_id })
    }
  ...
  private _set_exposed_sub_pin_color = (
    ...
    const name_comp = this._exposed_pin_name[ext_pin_node_id]
  ...
  private _sim_remove_component = (unit_id: string): void => {
    ...
    this._move_out_sub_component_children(unit_id, parent_id)
    ...
    const sub_component = this._get_sub_component(unit_id)
    if (sub_component) {
      const sub_component_spec_children =
  ...
  private _sim_refresh_exposed_pin_marker = (
    ...
    if (anchor_node_id) {
      ...
      this._set_exposed_pin_marker_to_default(
        type,
        ext_pin_node_id,
        int_pin_node_id
      )
  ...
  private __drag_start = (
    ...
    if (this._is_external_pin_node_id(node_id)) {
      ...
      if (this._has_node(exposed_int_node_id)) {
        this._negate_node_layer(exposed_int_node_id)
      }
    } else if (this._is_internal_pin_node_id(node_id)) {
      this._negate_node_layer(node_id)
      const { type, id, subPinId } = segmentExposedNodeId(node_id)
      const exposed_ext_node_id = getExternalNodeId(type, id, subPinId)
      if (this._has_node(exposed_ext_node_id)) {
        this._negate_node_layer(exposed_ext_node_id)
      }
    }
  }
  ...
  private _ascend_node = (node_id: string, pointer_id: number): void => {
    ...
    if (this._is_unit_node_id(node_id)) {
      ...
      const { id, type, subPinId } = segmentExposedNodeId(node_id)
      const int_pin_node_id = getInternalNodeId(type, id, subPinId)
      this._negate_node_layer(int_pin_node_id)
      this._ascend_node_z(int_pin_node_id)
    } else if (this._is_internal_pin_node_id(node_id)) {
      const { id, type, subPinId } = segmentExposedNodeId(node_id)
      const ext_pin_node_id = getExternalNodeId(type, id, subPinId)
      this._negate_node_layer(ext_pin_node_id)
      this._ascend_node_z(ext_pin_node_id)
    }
  }
  ...
  private _descend_node = (node_id: string): void => {
    ...
    if (this._is_unit_node_id(node_id)) {
      ...
      const { id, type, subPinId } = segmentExposedNodeId(node_id)
      const int_pin_node_id = getInternalNodeId(type, id, subPinId)
      this._descend_node_z(int_pin_node_id)
      this._refresh_node_layer(int_pin_node_id)
    } else if (this._is_internal_pin_node_id(node_id)) {
      const { id, type, subPinId } = segmentExposedNodeId(node_id)
      const ext_pin_node_id = getExternalNodeId(type, id, subPinId)
      this._descend_node_z(ext_pin_node_id)
      this._refresh_node_layer(ext_pin_node_id)
    }
  }
  ...
  private _start_long_press_collapse = (
    ...
    // TODO
    this._long_press_collapse_next_id_map = { unit: {}, merge: {} }
    ...
    for (const selected_node_id in this._selected_node_id) {
      ...
      if (this._is_merge_node_id(selected_node_id)) {
        const merge_node_id = selected_node_id
        ...
        if (merge_unit_count === 1) {
          ...
          if (merge_single_unit_id === node_id) {
    ...
    if (this._long_press_unit_collapsing) {
      ...
      if (node_id === null) {
        ...
        setSpec(new_spec_id, graph_unit_spec)
          ...
          (pin_node_id: string, type, pin_id) => {
            if (!this._long_press_collapse_node_id.has(pin_node_id)) {
              ...
              if (next_pin_id) {
                ...
                this._long_press_collapse_unit_next_pin_map[unit_id][type][
                  pin_id
                ] = {
                  pinId: next_pin_id,
      ...
      for (const merge_id of merge_ids) {
        let next_merge_id = merge_id
        if (graph_unit_spec.merges[merge_id]) {
      ...
      const long_press_link_pins = link_pin_ids.map((pin_node_id) => {
        const { unitId, type, pinId } = segmentLinkPinNodeId(pin_node_id)
        ...
        return { unitId, type, pinId, mergeId, oppositePinId }
      })
    ...
    if (this._long_press_unit_collapsing) {
      this._pod.$moveSubgraphInto({
        ...
        nodeIds: {
          ...
          linkPin: this._long_press_collapse_link_pins,
  ...
  private _long_press_collapse_node = (node_id: string): void => {
    ...
    if (this._long_press_unit_collapsing) {
      if (!this._long_press_collapse_unit_id) {
        ...
        const new_unit = {
          path: this._long_press_collapse_next_spec_id,
        }
  ...
  private _move_datum_into_array = (datum_node_id: string): void => {
    ...
    if (array_type === TreeNodeType.ArrayLiteral) {
      ...
      const datum = this._datum[this._long_press_collapse_datum_node_id]
  ...
  public _copy_nodes = (node_ids: string[]) => {
    ...
    for (const exposed_node_id of exposed_ids) {
      ...
      const sub_pin_spec = clone(
        this._get_exposed_sub_pin_spec(type, id, subPinId)
      )
      ...
      if (type === 'input') {
        inputs[id] = inputs[id] || { pin: {} }

        inputs[id].pin[subPinId] = sub_pin_spec
      } else {
        outputs[id] = outputs[id] || { pin: {} }

        outputs[id].pin[subPinId] = sub_pin_spec
      }
    }
  ...
  public paste_spec = (bundle: BundleSpec, position: Position) => {
    ...
    const map_spec_id: Dict<string> = injectSpecs(specs)
    ...
    for (const input_id in inputs) {
      ...
      for (const sub_pin_id in pin) {
        const new_sub_ext_pin_id = getExternalNodeId(
          'input',
          new_input_id,
          sub_pin_id
        )
  ...
  private _force_custom_layer_exposed = (alpha: number): void => {
    ...
    for (const a_id in this._exposed_int_unplugged) {
      ...
      const { id, type, subPinId } = segmentExposedNodeId(a_id)

      const b_id = getExternalNodeId(type, id, subPinId)
  ...
  private _on_graph_unit_add_unit_moment = (
    ...
    // TODO
    // Graph should not be the responsible for updating global specs
    ...
    next_spec = specReducer.addUnit(
      { id: added_unit_id, unit: { path: specId } },
      next_spec
    )
  ...
  private _ensure_fork_spec(init_spec_id: string): {
    ...
    const init_spec = getGraphSpec(init_spec_id)

    if (init_spec.fork) {
      return { spec_id: init_spec_id, spec: init_spec, forked: false }
    } else {
      const forked_spec_id = newSpecId()
  ...
  private _spec_set_unit_spec_id = (unit_id: string, spec_id: string): void => {
    this._spec.units[unit_id].path = spec_id
  }
