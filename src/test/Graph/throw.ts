...
composition2.addUnit(
  {
    path: THROW,
    catchErr: false,
    input: { message: { data: '"akbar"' } },
    output: {},
  },
  throwId1
)
assert.equal(composition2.getErr(), 'allahu')
composition2.removeUnit(throwId0)
assert.equal(composition2.getErr(), 'akbar')
...
composition4.addUnit({ path: THROW }, 'throw', throwUnit)
