...
assert.deepEqual(getTreeNodeType(IDENTITY), TreeNodeType.Unit)
...
assert(isTypeMatch('<T>', IDENTITY))
assert(isTypeMatch('any', IDENTITY))
assert(isTypeMatch(IDENTITY, 'any'))
...
assert(!isTypeMatch('null', IDENTITY))
assert(!isTypeMatch('object', 'class'))
assert(!isTypeMatch('number', 'class'))
assert(!isTypeMatch('number', IDENTITY))
assert(!isTypeMatch(IDENTITY, 'number'))
